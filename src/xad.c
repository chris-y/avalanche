...
static long xad_extract_file_private(void *awin, char *dest, struct xad_userdata *xu, struct xadDiskInfo *di, struct xadFileInfo *fi, ULONG (scan)(void *awin, char *file, UBYTE *buf, ULONG len, BOOL delete), ULONG *pud)
{
	long err = 0;
...
long xad_extract_file(void *awin, char *file, char *dest, struct Node *node, void *(getnode)(void *awin, struct Node *node), ULONG (scan)(void *awin, char *file, UBYTE *buf, ULONG len, BOOL delete), ULONG *pud)
{
	...
	return xad_extract_file_private(awin, dest, xu, di, fi, scan, pud);
...
long xad_extract_array(void *awin, ULONG total_items, char *dest, void **array, void *(getuserdata)(void *awin, struct Node *node), ULONG (scan)(void *awin, char *file, UBYTE *buf, ULONG len, BOOL delete))
{
	long err = XADERR_OK;
	ULONG pud = 0;
	struct xadFileInfo *fi = NULL;
	...
	if(xu->ai) {
		for(int i = 0; i < total_items; i++) {
			...
			err = xad_extract_file_private(awin, dest, xu, di, fi, scan, &pud);
			if(err != XADERR_OK) {
				if(err == XADERR_BREAK) err = XADERR_OK; // user abort
				return err;
			}
